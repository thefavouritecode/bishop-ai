...
# ── PREMIUM CSS (SIMPLIFIED & BULLETPROOF) ────────────────────────────────────
st.markdown("""
<style>
...
a[href*="streamlit.io"], [class*="streamlit"], [data-testid="stHeader"] { 
...
.sb-cross { font-size: 1.8rem; color: var(--gold); }
.sb-name { font-family: 'Playfair Display', serif; font-size: 1.1rem; font-weight: 700; color: var(--gold-lt); }
.sb-name small { display: block; font-size: 0.7rem; color: var(--txt3); margin-top: 4px; }
...
[data-testid="stSidebar"] button:hover {
  background: rgba(232,192,106,0.1) !important;
  border-color: var(--gold) !important;
}

.sec { font-size: 0.7rem; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: var(--gold-dk); margin: 1rem 0 0.5rem; }
...
.tb-cross { font-size: 1.5rem; color: var(--gold); }
...
.wc { font-size: 3.5rem; color: var(--gold); margin-bottom: 1.5rem; }
.wt { font-family: 'Playfair Display', serif; font-size: 2rem; font-weight: 700; color: var(--gold-lt); margin-bottom: 0.5rem; }
.wr { width: 60px; height: 2px; margin: 1rem auto; background: var(--gold); }
...
[data-testid="stChatInputContainer"] > div {
  ...
  border: 1.5px solid rgba(232,192,106,0.3) !important;
  border-radius: var(--pill) !important;
  box-shadow: 0 0 20px rgba(232,192,106,0.1) !important;
}
[data-testid="stChatInputContainer"] > div:focus-within {
  border-color: var(--gold) !important;
  box-shadow: 0 0 30px rgba(232,192,106,0.25) !important;
}
</style>
    ...
    if os.path.exists("media"):
        ...
        if pdfs:
            ...
            for pdf in pdfs:
                ...
                with col2:
                    st.button("👁️ Preview", key=f"prev_{pdf}", use_container_width=True)
                st.markdown("---")
        ...
        if audios:
            ...
            for audio in audios:
                ...
                with col2:
                    st.button("▶️ Listen", key=f"prev_{audio}", use_container_width=True)
                st.markdown("---")
    else:
